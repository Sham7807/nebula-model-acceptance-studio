...
另提供独立的 **小小宇宙无敌 · Nebula Studio** Mac 应用：原生侧边栏、概览与连接设置，内置检测引擎，支持本机历史数据库、钥匙串和原生文件上传 / 报告保存。网页版代码和现有部署方式保持完整。
...
Claude、CCMax 与 KVV 可追加有预算的业务验证：长输出、长上下文、长流式、图像理解、推理、多轮工具和自定义业务样本，记录首次成功、重试恢复、分时段表现及实际并发。**能力得分与接入建议分别计算**；关键业务异常不能靠其他项目的高分抵消。Claude 新配置默认 2 分钟快速筛查，CCMax/KVV 主动开启，旧 API 不传生产配置保持关闭。

请求预算包含工具每一轮、重试和控制请求；报价为空表示未知。按逐请求 usage 估算费用，也可导入上游账单并保存到历史，查看包含失败和重试的每个成功任务成本。报告保留统一样式，增加独立准入条件、已验证范围与成本明细。参见[生产验收与费用对账](docs/production-acceptance.md)。
...
原专项的快速验收执行 11 次请求（1 次签名、3 次 SSE、1 次强制工具、1 次非法模型和 5 次高级探针）；批量验收执行 62 次请求（5 次签名、50 次 SSE、其余 7 次固定探针）。**新任务还默认追加标准参数矩阵**，因此页面总用量高于上述原专项数量。开始前可预览矩阵请求和用量，实际请求数以报告为准。
...
| 输出上限 | 基础截断与非法值；追加矩阵默认覆盖 1 / 10 / 20、双场景和流式对照 | 10% |
...
开始前点击 **查看测试请求**，按模块查看原专项与追加矩阵的请求体、条件依赖和预计请求数；预览不发送 API Key、不调用模型。原专项和追加矩阵分别勾选检测范围，必要的成功基线用于解释负对照。工具结果和签名回传依赖上一轮有效结果，实际请求数会单独记录。
...
thinking 签名采用 adaptive 配置；仅在上游明确表示不支持时追加 enabled 兼容对照。思考耗尽输出预算不等于协议失败：合法截断响应即使没有正文，也按 usage 和结束原因验收；需要正文的语义检查则标明预算不足，不冒充通过。
...
通用、CCMax、Claude 与 KVV 均支持多参数验收。通用检测使用自己的快速 / 标准 / 完整方案；三个服务端专项默认选择「标准矩阵」，在原专项之后执行。KVV 官方结果与工作台新增探针分别保留，不修改官方断言来提高分数。

| 方案 | 截断档位 | 长输出场景 × 流式模式 × 轮数 | 截断变体 |
| --- | --- | --- | ---: |
| 快速 | 1、10、20 | 1 × 非流式 × 1 | 3 |
| 标准 | 1、10、20 | 2 × 流式 / 非流式 × 1 | 12 |
| 完整 | 1、10、20、64、128、256 | 3 × 流式 / 非流式 × 2 | 72 |

此外包含较大输出预算的长回答对照、工具选择与 Schema 变体、已知答案图片与多图顺序、合成注入对照、大前缀冷暖缓存及阶梯压测。每条变体独立记录参数、使用量、结束原因、证据与结论；压测区分请求完成率与内容匹配率。

**完整矩阵会显著增加模型用量。** 服务端矩阵全选时，快速 / 标准 / 完整最多追加 27 / 59 / 160 次请求，其中工具回填为条件请求；数量不包含原专项和其他批量模型。可单独关闭矩阵模块，或选择仅运行原专项。具体负载、缓存规模、评分例子和 API 用法见 [参数矩阵与报告判读](docs/acceptance-matrices.md)。
...
模型名中的路径、控制字符和系统保留字符会自动替换；时间包含年月日、时分秒，便于区分不同测试批次。基础、通用、CCMax、Claude、KVV、GPT 和历史下载使用同一报告标准：蓝色封面 → **结论与耗时 → 验收模块总览 → 能力评分** → 逐项方法/预期/实测、问题影响与建议、请求证据 → **末尾全项测试清单**。服务端与独立 HTML 共用 `report-theme.css`，可打印为 PDF；服务端报告支持搜索和筛选。

报告只展示一个模块加权综合分，维度分用于解释能力表现，对照与汇总不重复计分。顶部结论关联实际证据，区分输出超限、非法参数校验失败和指令隔离风险；注入探针失败不能证明上游偷偷添加提示词。缓存单列**暖请求 Token 复用率**（有效暖请求缓存读取总量 ÷ 总输入量），排除冷请求和改前缀对照，缺失字段不算零。

资源评级按本轮综合分划分：**90–100 分为优质资源，70–89 分为中等资源，低于 70 分为低等级资源**；无可判定分数时显示待评估。可判定率低于 80%、可评分模块权重低于 70%，或明确关联能力请求少于 10 条时标注「暂定」。多模型汇总评级仅供参考，各模型需独立判读。摘要区分「多数检查通过 · 少量异常」「需重点核查」和「证据待补齐」，缓存零命中说明为「本轮未观察到复用」；原始异常、分数和证据不变。

耗时包括测试起止时间、实际总耗时、单请求 P50/P95、压测阶段耗时及每条请求延迟。并发请求时长不相加；多记录使用最早开始至最晚结束的完整区间，包含记录间隔。旧数据缺字段时显示「未记录」，只有请求时间依据时明确标为「请求观测窗口」，不冒充全程耗时。

实际请求数与检查项数分别统计，只有真实关联的证据才链接到检查项。统一报告同时显示 **已判定通过率** 与 **可判定率**：前者在通过 / 失败项内计算，后者说明多少适用项已有明确结论；无法判定不再获得固定分数，全部未知显示「—」。例如 1 项通过、1 项证据不足，已判定通过率为 100%，可判定率为 50%，不能据此宣称整个能力已验证。来源真实性等观察项另列，不充当能力扣分；未选、不适用和未覆盖项同样明确展示。维度保留场景数、参数组合数、实际请求数和小样本提示，逐项展开可读参数及原始证据。
