...
工作台支持浏览器直连的基础测试与通用深度检测，也支持由本地 Python 服务执行的 CCMax 渠道验收和 Kimi Vendor Verifier（KVV）。完整工作台中的模型列表统一通过服务获取。它是渠道兼容性与传输质量工具，不是模型身份认证、生成质量评分或“覆盖所有私有接口”的承诺；每项结论都以实际请求和报告中的证据为准。
