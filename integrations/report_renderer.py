...
def render_report(result, directory=None):
    ...
    score_cards=''.join('<article class="score-dimension '+esc(d.get('status','not_covered'))+'"><div class="score-dimension-head"><span>'+esc(d.get('label'))+'</span><b>'+esc(d.get('score',0))+'<small>/100</small></b></div><div class="bar"><i style="width:'+str(max(0,min(100,int(d.get('score',0)))))+'%"></i></div><small>'+esc('覆盖 '+str(d.get('covered',0))+' 项 · '+', '.join(str(k)+': '+str(v) for k,v in (d.get('counts') or {}).items() if v))+'</small></article>' for d in score_dims)
