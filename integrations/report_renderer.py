...
def render_report(result, directory=None):
    ...
    def score_evidence(d):
        ...
        summary='覆盖 %s 项 · %s' % (d.get('covered',0), ', '.join(str(k)+': '+str(v) for k,v in counts.items() if v))
        ids=[]
        for check_id in d.get('check_ids') or []:
            anchor=check_anchors.get(check_id)
            if anchor:
                ids.append('<a href="#'+anchor+'">'+esc(check_id)+'</a>')
        return '<small>'+esc(summary)+'</small>'+('<div class="score-evidence">评分依据：'+'、'.join(ids)+'</div>' if ids else '<div class="score-evidence">本轮没有可计分证据</div>')
    score_cards=''.join('<article class="score-dimension '+esc(d.get('status','not_covered'))+'"><div class="score-dimension-head"><span>'+esc(d.get('label'))+'</span><b>'+esc(d.get('score',0))+'<small>/100</small></b></div><div class="bar"><i style="width:'+str(max(0,min(100,int(d.get('score',0)))))+'%"></i></div>'+score_evidence(d)+'</article>' for d in score_dims)
    score_html='<section class="score-panel" id="score"><div class="score-head"><div><span class="index">SCORE / DIMENSIONS</span><h2>能力评分与覆盖明细</h2><p class="score-note">'+esc(score.get('method',''))+'</p></div><div class="score-total">'+esc(score.get('total',0))+'<small> / 100</small></div></div><div class="score-grid">'+score_cards+'</div><ul class="recommendations">'+''.join('<li>'+prose(x)+'</li>' for x in (score.get('recommendations') or []))+'</ul></section>'
    ...
    for i,c in enumerate(checks):
        check_html.append('<article class="check" id="check-'+str(i+1)+'" data-status="'+esc(c.get('status','inconclusive'))+'"><div class="check-head"><div><h3>'+f'{i+1:02d} · '+esc(c.get('title') or c.get('id'))+'</h3><div class="case-id">'+esc(c.get('id',''))+'</div></div>'+badge(c.get('status'))+'</div>'
