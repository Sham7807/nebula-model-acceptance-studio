...
新增独立任务中心：新建任务会打开干净的工作区，不会自动恢复上一份已完成报告；已有报告仍在测试档案中。每个任务持有自己的渠道地址、密钥、模型、WebKit 页面和运行状态，切换侧边栏或任务中心不会打断请求。桌面引擎最多并行运行 4 个顶层验收任务，可以分别查看进度、停止任务、继续查看结果或关闭已结束页面。退出应用仍会停止本机任务，并明确提示这一点。

网页端默认仍保持单任务限制，避免改变服务器部署行为；并行能力只对 macOS 本地引擎开放。报告、历史数据库和钥匙串数据路径保持兼容。
...
同步网站版本 `325e7d3`：统一报告将证据结论、实际耗时和模块评分提前；使用唯一模块加权综合分；新增暖请求 Token 复用率、单请求 P50/P95 与压测阶段耗时。全项清单移至报告末尾，缺少依据的指标明确显示未记录。替换应用即可更新，不会迁移或删除本机历史、渠道偏好和钥匙串凭据。
