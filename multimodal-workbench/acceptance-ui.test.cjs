  ...
  try {
    ...
    }
    {
      ...
      try {
        ...
        await picker.getByRole('option', { name: 'kimi-k3', exact: true }).click();
        assert.equal(await page.locator('#acceptanceModel').inputValue(), 'kimi-k3');
        await picker.getByRole('button', { name: '选择渠道模型' }).click();
        await picker.getByRole('option', { name: 'ccmax', exact: true }).click();
        ...
        await page.locator('#acceptanceAuth').selectOption('bearer');
        assert.equal(await picker.getByRole('option').count(), 0, 'auth change clears previous catalog');
    ...
    }
    {
      ...
      try {
        ...
        await page.locator('#acceptanceModels').click();await waitModelIdle(page);
        const picker = page.locator('.choice-picker').filter({ has: page.locator('#acceptanceModel') });
        assert.equal(await picker.getByRole('option', { name: 'new-model-a' }).count(), 1);
        ...
        assert.equal(await picker.getByRole('option', { name: 'stale-model' }).count(), 0, 'late old-channel response cannot replace the new list');
        assert.equal(await picker.getByRole('option', { name: 'new-model-b' }).count(), 1);
        assert.equal(await page.locator('#acceptanceModel').inputValue(), 'preserved-manual-id');
