...
async function exportReport(){
  ...
  setBusy(true);$('stopBtn').disabled=true;$('exportBtn').disabled=true;$('runningLabel').textContent='正在导出报告';$('runningDetail').textContent='整理本次记录与证据，不会重新请求渠道。';
  try{
    if(!window.WorkbenchReport?.renderExport)throw new Error('统一报告模块未加载，请刷新页面后重试。');
    const html=await window.WorkbenchReport.renderExport(reportRecords,reportLogs);
    const models=[...new Set(reportRecords.map(r=>r.model).filter(Boolean))],modelPart=(models.join('、')||'未命名模型').replace(/[\\/:*?"<>|\u0000-\u001f]+/g,'-').slice(0,80);
    const now=new Date(),pad=v=>String(v).padStart(2,'0'),stamp=`${now.getFullYear()}${pad(now.getMonth()+1)}${pad(now.getDate())}-${pad(now.getHours())}${pad(now.getMinutes())}${pad(now.getSeconds())}`;
    downloadBlob(new Blob([html],{type:'text/html;charset=utf-8'}),`测试报告-${modelPart}-${stamp}.html`);notify('统一报告已下载。');
