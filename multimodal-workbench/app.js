...
async function exportReport(){
  ...
  try{
    ...
    const report=`<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="referrer" content="no-referrer"><title>小小宇宙无敌 · 渠道测试报告</title><style>:root{--ink:#233d32;--muted:#6e7e73;--line:#dce5dc;--paper:#f4f7f2;--green:#28664e;--red:#ac4b3d;--amber:#936b22}*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font:14px/1.75 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}main{max-width:1180px;margin:auto;padding:28px 22px 56px}h1,h2,h3,p{margin:0}h1{font-size:32px;line-height:1.3}h2{font-size:19px}h3{font-size:14px}.muted{color:var(--muted);font-size:11px}.mast{display:flex;justify-content:space-between;align-items:center;padding:0 0 16px}.brand{font-weight:650;font-size:16px}.brand:before{content:'宇';display:inline-grid;place-items:center;width:32px;height:32px;color:#fff;background:var(--ink);border-radius:10px;margin-right:9px}.cover{color:#f8fbf7;background:var(--ink);padding:30px;border-radius:18px}.cover p{color:#c6d5c9;margin-top:6px}.nav{display:flex;gap:17px;flex-wrap:wrap;border-bottom:1px solid var(--line);padding:15px 0}.nav a{color:var(--green);font-size:12px}.panel,.result{background:#fff;border:1px solid var(--line);border-radius:14px;padding:21px;margin-top:18px}.metrics{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}.metric{padding:15px 16px;border:1px solid var(--line);border-radius:10px;background:#fff}.metric b{display:block;font-size:28px}.metric span{font-size:11px;color:var(--muted)}.score-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:13px}.score{padding:13px;border:1px solid var(--line);border-radius:10px;background:#fbfcfa}.score>div{display:flex;justify-content:space-between;gap:8px}.score strong{font-size:18px}.score strong small{font-size:10px;color:var(--muted)}.score>i{display:block;height:5px;background:#e6ece4;border-radius:5px;margin:8px 0;overflow:hidden}.score>i em{display:block;height:100%;background:#5f916d}.score.failed>i em{background:#bb6453}.score.inconclusive>i em{background:#c1994b}.score p{font-size:10px;color:var(--muted)}.recommendations{font-size:11px;color:var(--muted);padding-left:18px}.recommendations li+li{margin-top:5px}.badge{display:inline-block;padding:3px 8px;border-radius:20px;font-size:10px;background:#edf0ea;color:#627363}.badge.success{background:#e9f3e9;color:#28664e}.badge.error,.badge.stopped{background:#faebe6;color:#a0483c}.badge.pending,.badge.unrecognized{background:#faf1da;color:#86611d}.result header{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.result header p{margin-top:5px}.result section{margin-top:13px}.result pre,.panel pre{white-space:pre-wrap;overflow-wrap:anywhere;max-height:450px;overflow:auto;background:#f4f7f1;border:1px solid #e0e8dc;border-radius:8px;padding:13px;font:11px/1.75 ui-monospace,SFMono-Regular,monospace}.result .error{background:#fdf2ee;color:#a0483c}.media{display:flex;flex-wrap:wrap;gap:13px;margin-top:14px}.media figure{margin:0;flex:1 1 280px}.media img,.media video{width:100%;max-height:520px;object-fit:contain;border-radius:7px;background:#f0f3eb}.media audio{width:100%}.diagnostic{border:1px solid #eadfbd;border-radius:9px;background:#fffbf1;color:#7e692f;padding:12px;margin-top:13px}.diagnostic p{margin:5px 0;white-space:pre-wrap}.result details{margin-top:14px}.result summary{cursor:pointer;color:var(--green);font-size:11px}a{color:var(--green)}footer{margin-top:25px;border-top:1px solid var(--line);padding-top:15px;color:var(--muted);font-size:10px}@media(max-width:700px){main{padding:18px 13px}.metrics{grid-template-columns:1fr 1fr}.score-grid{grid-template-columns:1fr}.result header{display:block}.cover{padding:22px}h1{font-size:25px}}@media print{body{background:#fff}.nav{display:none}.result,.panel{break-inside:avoid}.result details{display:block}}</style></head><body><main><div class="mast"><span class="brand">小小宇宙无敌</span><span class="muted">CHANNEL ACCEPTANCE REPORT</span></div><header class="cover"><h1>多模态渠道测试报告</h1><p>本次会话 · ${escapeHtml(new Date().toLocaleString('zh-CN'))} · ${reportRecords.length} 条记录 · API Key 已隐藏</p><p>生成成功不等于能力合格；评分只基于本次实际请求与保存证据。</p></header><nav class="nav"><a href="#overview">结论总览</a><a href="#score">能力评分</a><a href="#records">逐条结果</a><a href="#logs">执行日志</a></nav><section id="overview" class="panel"><h2>结论总览</h2><div class="metrics"><div class="metric"><span>总分</span><b>${totalScore}<small>/100</small></b></div><div class="metric"><span>测试记录</span><b>${reportRecords.length}</b></div><div class="metric"><span>返回成功</span><b>${reportRecords.filter(r=>r.status==='success').length}</b></div><div class="metric"><span>失败 / 待判定</span><b>${reportRecords.filter(r=>['error','stopped'].includes(r.status)).length} / ${reportRecords.filter(r=>!['success','error','stopped'].includes(r.status)).length}</b></div></div><p class="muted" style="margin-top:10px">评分方法：各已覆盖维度等权；成功=100%，等待或未识别=40%，失败/停止=0%；未覆盖维度不计入总分。</p></section><section id="score" class="panel"><h2>能力评分与明细</h2><div class="score-grid">${scoreCards}</div><ul class="recommendations">${recommendations.map(x=>`<li>${escapeHtml(x)}</li>`).join('')}</ul></section><section id="records"><h2 style="margin-top:22px">逐条结果与证据</h2>${content}</section><section id="logs" class="panel"><h2>执行日志</h2><pre>${escapeHtml(scrub(reportLogs.map(l=>`[${l.time}] ${l.message}`).join('\n')))}</pre></section><footer>小小宇宙无敌 · 独立 HTML · 无外部依赖 · 凭据已隐藏</footer></main></body></html>`;
