...
async function exportReport(){
  ...
  try{
    ...
    let content='';for(const r of reportRecords){const media=await Promise.all((r.media||[]).map(mediaHTML));const diagnostic=r.error?diagnoseResult(r):null;const diagnosticHtml=diagnostic?`<section class="diagnostic"><h3>错误诊断与处理建议</h3>${[...diagnostic.querySelectorAll('p')].map(item=>`<p>${escapeHtml(item.textContent)}</p>`).join('')}</section>`:'';const score=dims.find(d=>d.covered&&((d.id==='multimodal'&&r.kind!=='text')||d.id==='protocol'));content+=`<article class="result"><header><h2>${escapeHtml(r.model||'未命名模型')} <span class="badge ${escapeHtml(r.status)}">${escapeHtml(statusLabel[r.status]||r.status)}</span></h2><p class="muted">${escapeHtml(r.preset||'未记录')} · ${escapeHtml(new Date(r.createdAt||Date.now()).toLocaleString('zh-CN'))} · ${((r.elapsedMs||0)/1000).toFixed(2)} 秒 · ${r.requests?.length||0} 次 HTTP 请求</p></header>${r.taskId?`<p>任务 ID：${escapeHtml(r.taskId)}</p>`:''}${r.error?`<pre class="error">${escapeHtml(scrub(r.error))}</pre>`:''}${diagnosticHtml}${r.text?`<section><h3>文本结果</h3><pre>${escapeHtml(scrub(r.text))}</pre></section>`:''}<div class="media">${media.join('')}</div><details><summary>请求配置、原始响应与请求记录</summary><pre>${escapeHtml(JSON.stringify(scrub({config:r.config,response:r.raw,requests:r.requests},true),null,2))}</pre></details></article>`;}
    const report=`<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="referrer" content="no-referrer"><title>小小宇宙无敌 · 渠道测试报告</title><style>:root{--ink:#233d32;--muted:#6e7e73;--line:#dce5dc;--paper:#f4f7f2;--green:#28664e;--green-soft:#e9f3e9;--red:#ac4b3d;--red-soft:#faebe6;--amber:#936b22;--amber-soft:#faf1da;--card:#fff}*{box-sizing:border-box}html{scroll-behavior:smooth;scroll-padding-top:76px}body{margin:0;background:var(--paper);color:var(--ink);font:14px/1.75 -apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC",sans-serif}main{max-width:1200px;margin:auto;padding:32px 28px 60px}h1,h2,h3,p{margin:0}h1{font-size:32px;line-height:1.3;letter-spacing:-.8px;overflow-wrap:anywhere}h2{font-size:21px;line-height:1.4}h3{font-size:16px;line-height:1.5}.muted{color:var(--muted);font-size:11px}.mast{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:0 0 18px}.brand{font-weight:650;font-size:16px}.brand:before{content:'宇';display:inline-grid;place-items:center;width:32px;height:32px;color:#fff;background:var(--ink);border-radius:10px;margin-right:10px}.cover{color:#f8fbf7;background:var(--ink);padding:32px;border-radius:20px}.cover p{color:#c6d5c9;margin-top:10px}.nav{display:flex;gap:20px;flex-wrap:wrap;background:#f4f7f2ed;backdrop-filter:blur(10px);border-bottom:1px solid var(--line);padding:17px 0;position:sticky;top:0;z-index:2;font-size:12px}.nav a{color:var(--green);white-space:nowrap;text-decoration:none}.panel,.result{background:var(--card);border:1px solid var(--line);border-radius:14px;padding:22px;margin-top:18px;box-shadow:0 2px 8px rgba(35,61,50,.025)}.metrics{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-top:16px}.metric{padding:18px 20px;background:#fff;border:1px solid var(--line);border-radius:12px}.metric b{display:block;font-size:30px;line-height:1.3;font-variant-numeric:tabular-nums}.metric b small{font-size:12px;color:var(--muted);font-weight:500}.metric span{font-size:12px;color:var(--muted)}.score-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:15px}.score{padding:13px;border:1px solid var(--line);border-radius:10px;background:#fbfcfa}.score>div{display:flex;justify-content:space-between;gap:8px}.score strong{font-size:18px}.score strong small{font-size:10px;color:var(--muted)}.score>i{display:block;height:5px;background:#e6ece4;border-radius:5px;margin:8px 0;overflow:hidden}.score>i em{display:block;height:100%;background:#5f916d}.score.failed>i em{background:#bb6453}.score.inconclusive>i em{background:#c1994b}.score.not_covered{opacity:.75}.score p{font-size:10px;color:var(--muted)}.recommendations{font-size:11px;color:var(--muted);padding-left:18px;margin:14px 0 0}.recommendations li+li{margin-top:5px}.badge{display:inline-block;white-space:nowrap;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600;background:#eef1ed;color:#627363}.badge.success{background:var(--green-soft);color:var(--green)}.badge.error,.badge.stopped{background:var(--red-soft);color:#a0483c}.badge.pending,.badge.unrecognized{background:var(--amber-soft);color:#86611d}.result header{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.result header p{margin-top:5px}.result section{margin-top:13px}.result pre,.panel pre{white-space:pre-wrap;overflow-wrap:anywhere;max-height:480px;overflow:auto;background:#f4f7f1;border:1px solid #e0e8dc;border-radius:8px;padding:14px;font:11px/1.75 ui-monospace,SFMono-Regular,monospace;color:#374d3b}.result .error{background:#fdf2ee;color:#a0483c;border-color:#efd5ce}.media{display:flex;flex-wrap:wrap;gap:13px;margin-top:14px}.media figure{margin:0;flex:1 1 280px;padding:10px;border:1px solid var(--line);border-radius:10px;background:#fbfcfa}.media img,.media video{display:block;width:100%;max-height:520px;object-fit:contain;border-radius:7px;background:#f0f3eb}.media audio{width:100%}.media figcaption{font-size:11px;margin-top:8px}.diagnostic{border:1px solid #eadfbd;border-radius:9px;background:#fffbf1;color:#7e692f;padding:14px;margin-top:13px}.diagnostic h3{font-size:14px;margin-bottom:6px;color:#715b25}.diagnostic p{margin:5px 0;white-space:pre-wrap}.result details{margin-top:14px;border-top:1px solid #e8ede5;padding-top:10px}.result summary{cursor:pointer;color:var(--green);font-size:12px}.result summary::marker{color:#7f9579}a{color:var(--green)}footer{display:flex;justify-content:space-between;gap:16px;margin-top:26px;border-top:1px solid var(--line);padding-top:18px;color:var(--muted);font-size:11px}@media(max-width:700px){main{padding:18px 14px 35px}.mast{padding-bottom:14px}.cover{padding:23px 20px;border-radius:14px}h1{font-size:25px}.metrics{grid-template-columns:1fr 1fr;gap:8px}.metric{padding:14px}.metric b{font-size:25px}.score-grid{grid-template-columns:1fr 1fr}.result header{display:block}.result header .badge{margin-top:8px}.panel,.result{padding:17px}.nav{gap:14px;padding:13px 0}}@media(max-width:480px){.score-grid{grid-template-columns:1fr}.metrics{grid-template-columns:1fr 1fr}.metric b{font-size:22px}footer{display:block}.footer span{display:block}}@media print{body{background:#fff;font-size:10px}main{max-width:none;padding:0}.nav{display:none}.cover{background:#fff;color:var(--ink);border:1px solid var(--line);padding:20px}.cover p{color:var(--muted)}.panel,.result{break-inside:avoid;box-shadow:none}.result details{display:block}.media figure{break-inside:avoid}.result pre,.panel pre{max-height:none;overflow:visible;font-size:8px}footer{display:flex}}</style></head><body><main><div class="mast"><span class="brand">小小宇宙无敌</span><span class="muted">CHANNEL ACCEPTANCE REPORT</span></div><header class="cover"><h1>多模态渠道测试报告</h1><p>本次会话 · ${escapeHtml(new Date().toLocaleString('zh-CN'))} · ${reportRecords.length} 条记录 · API Key 已隐藏</p><p>生成成功不等于能力合格；评分只基于本次实际请求与保存证据。</p></header><nav class="nav"><a href="#overview">结论总览</a><a href="#score">能力评分</a><a href="#records">逐条结果</a><a href="#logs">执行日志</a></nav><section id="overview" class="panel"><h2>结论总览</h2><div class="metrics"><div class="metric"><span>总分</span><b>${totalScore}<small>/100</small></b></div><div class="metric"><span>测试记录</span><b>${reportRecords.length}</b></div><div class="metric"><span>返回成功</span><b>${reportRecords.filter(r=>r.status==='success').length}</b></div><div class="metric"><span>失败 / 待判定</span><b>${reportRecords.filter(r=>['error','stopped'].includes(r.status)).length} / ${reportRecords.filter(r=>!['success','error','stopped'].includes(r.status)).length}</b></div></div><p class="muted" style="margin-top:10px">评分方法：各已覆盖维度等权；成功=100%，等待或未识别=40%，失败/停止=0%；未覆盖维度不计入总分。</p></section><section id="score" class="panel"><h2>能力评分与明细</h2><div class="score-grid">${scoreCards}</div><ul class="recommendations">${recommendations.map(x=>`<li>${escapeHtml(x)}</li>`).join('')}</ul></section><section id="records"><h2 style="margin-top:22px">逐条结果与证据</h2>${content}</section><section id="logs" class="panel"><h2>执行日志</h2><pre>${escapeHtml(scrub(reportLogs.map(l=>`[${l.time}] ${l.message}`).join('\n')))}</pre></section><footer>小小宇宙无敌 · 独立 HTML · 无外部依赖 · 凭据已隐藏</footer></main></body></html>`;
