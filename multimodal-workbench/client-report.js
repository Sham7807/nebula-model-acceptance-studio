...
(function () {
  ...
  function cacheBrief(cases){
    ...
    const text=percent===null?`缓存命中率未知：${warm.size?`已关联 ${warm.size} 个暖请求，缺少完整缓存读取 / 输入计量。`:'没有明确关联暖请求及完整 usage。'}缓存模块分数不等于缓存命中率。`:`${complete<warm.size?'已计量暖请求':'暖请求'}缓存 Token 命中率 ${compactNumber(percent)}%（读取 ${read.toLocaleString('en-US')} / 完整输入 ${input.toLocaleString('en-US')} Token）；命中请求 ${hits}/${complete}，字段完整 ${complete}/${warm.size} 个暖请求。`;
    return {id:'cache',label:'缓存复用',status:percent===null||complete<warm.size||percent===0?'inconclusive':'passed',text,check_ids:[...new Set(used.length?used:matched.map(x=>x.id))],percent};
  }
  ...
  function executive(cases,requests,records){
    ...
    const add=(id,label,selected,positive,negative)=>{
      const s=scoreGroup(selected),text=s.failed?`${negative}：已判定 ${s.scored.length} 项中 ${s.failed} 项失败。`:s.scored.length?`${positive}：${s.passed}/${s.scored.length} 项已判定检查通过。`:`${label}证据不足，尚不能确认。`;
      items.push({id,label,status:s.status,text:text+(s.sampled.length>s.scored.length?` 另有 ${s.sampled.length-s.scored.length} 项待判定。`:''),check_ids:selected.filter(x=>x.status==='failed').concat(selected).map(x=>x.id)});
    };
    add('protocol','基础能力',cases.filter(x=>moduleId(x)==='protocol'),'基础接口可用','基础接口存在异常');
    add('tools_media','工具与多模态',cases.filter(x=>x.dims.includes('tools')||x.dims.includes('multimodal')),'已测工具 / 多模态可用','工具 / 多模态存在失败');
    ...
    const limitText=capFailure.length?`发现 ${capFailure.length} 个合法上限样本输出超限，长度控制未遵守请求。`:limits.failed?`${invalidFailures.length===limits.failed?'非法限长参数校验存在失败，不能据此判定合法限长失效':'限长测试存在失败，需按参数与错误原因复核'}（${limits.failed}/${limits.scored.length} 项已判定）。`:limits.scored.length?`本轮限长检查通过 ${limits.passed}/${limits.scored.length} 项。${limits.sampled.length>limits.scored.length?'仍有未触及截断或证据不足的样本。':''}`:'限长证据不足，尚不能确认截断是否生效。';
    items.push({id:'max_tokens',label:'长度控制',status:limits.status,text:limitText,check_ids:limited.filter(x=>x.status==='failed').concat(limited).map(x=>x.id)});
    const isolation=cases.filter(x=>x.dims.includes('security')),is=scoreGroup(isolation);
    items.push({id:'security',label:'指令隔离',status:is.status,text:(is.failed?`指令隔离存在异常，${is.failed}/${is.scored.length} 项已判定检查失败。`:is.scored.length?`本轮隔离检查 ${is.passed}/${is.scored.length} 项通过。`:'指令隔离证据不足。')+'不能据此认定上游私自添加提示词。',check_ids:isolation.filter(x=>x.status==='failed').concat(isolation).map(x=>x.id)});
    items.push(cacheBrief(cases));
    add('reliability','负载与稳定性',cases.filter(x=>x.dims.includes('reliability')),'本轮已测负载通过','本轮负载存在失败');
    const distinctModels=[...new Set(cases.map(x=>x.model))];
    if(distinctModels.length>1)return {items:[{id:'batch',label:'多模型结果',status:stats.failed?'failed':'inconclusive',text:'各模型独立判读。失败和缓存率请按对应模型检查证据阅读；跨模型汇总不能证明单个模型能力。',check_ids:cases.filter(x=>x.status==='failed').map(x=>x.id)}],headline:'多模型报告：各模型独立判读',detail:`包含 ${distinctModels.length} 个模型。综合分汇总本报告证据，逐项结果保留模型归属。`,resolution:stats.sampled.length?roundScore(stats.scored.length/stats.sampled.length*100):null,timing:timing(records,requests)};
    ...
    const headline=warnings.length?[(basic.status==='passed'?'基础接口可用':''),...warnings.slice(0,cachePhrase?2:3).map(x=>x.id==='max_tokens'&&capFailure.length?'限长出现超限':x.id==='security'?'指令隔离异常':x.label+'存在失败'),cachePhrase].filter(Boolean).slice(0,4).join('；'):[stats.scored.length?'已判定检查通过'+(stats.sampled.length>stats.scored.length?'，部分能力证据待补齐':'，以本轮覆盖范围为准'):'尚无可判定能力证据',cachePhrase].filter(Boolean).join('；');
    return {items,headline,detail:`${stats.passed} 项通过，${stats.failed} 项失败；${stats.sampled.length-stats.scored.length} 项无法判定。未执行与来源观察单独保留，缓存比例依据实际暖请求计量。`,resolution:stats.sampled.length?roundScore(stats.scored.length/stats.sampled.length*100):null,timing:timing(records,requests)};
  }
