...
(function () {
  ...
  const DIMENSIONS = [['multimodal','多模态能力'],['tools','工具调用'],['max_tokens','max_tokens / 长度控制'],['cache','缓存与 usage'],['protocol','协议与错误'],['reliability','稳定性与性能']];
  ...
  const dimensions = check => {
    ...
    if(explicit.length)return [...new Set(explicit.filter(key=>DIMENSIONS.some(d=>d[0]===key)))];
  ...
  const eligible=x=>x.applicable&&!x.localOnly&&['passed','failed','inconclusive'].includes(x.status);
  const points=x=>x.status==='passed'?1:x.status==='inconclusive'?.4:0;
  ...
  async function render(records, logs) {
    ...
    const dims=DIMENSIONS.map(([key,label])=>{
      ...
      const status=!scored.length?'not_covered':scored.some(x=>x.status==='failed')?'failed':scored.some(x=>x.status==='inconclusive')?'inconclusive':'passed';
      return {key,label,matched,scored,value,status};
    });
